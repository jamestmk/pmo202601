  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-8">
        ...
        {user.globalRole === "ADMIN" ? (
          <section className="mt-8 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            <form action={createProjectForm} className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="text-zinc-600">项目 owner（可选）</span>
                <select name="ownerId" className="rounded-md border border-zinc-300 px-3 py-2">
        ...
        <div className="mt-8">
          <ProjectsView projects={projects} isAdmin={user.globalRole === "ADMIN"} />
        </div>
      </main>
    </>
  );
